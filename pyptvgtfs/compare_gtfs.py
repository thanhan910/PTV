...
def compare_two_df(df1: pd.DataFrame, df2: pd.DataFrame, table_name = None, mode_id = None, columns = None) -> tuple[int, int, int]:
    '''Return a touple of (num_rows_in_df1_and_df2, num_rows_in_df1_not_df2, num_rows_in_df2_not_df1)'''
    if columns is None:
        columns = list(df1.columns)

    logging.debug(f'Comparing {len(df1)} rows in {table_name} to {len(df2)} rows in {table_name}, mode_id={mode_id}')
    logging.info(f'Comparing {len(df1)} rows in {table_name} to {len(df2)} rows in {table_name}, mode_id={mode_id}')
...
def compare_ptv_gtfs_versions(gtfs_v1 : list[GtfsDataframe], gtfs_v2 : list[GtfsDataframe]):
    ...
    dfm = pd.merge(df_v1, df_v2, how='outer', on=['table_name', 'mode_id'], suffixes=('_v1', '_v2'))
